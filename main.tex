...
Dynamic program analysis it is often the only way to analyse programs written in dynamic programming languages as their behavior makes static analysis loose its imprecision to the point is becomes useless.
Dynamic analysis however requires both a code and its input.
Some input can obtained from existing code such as tests or examples, however, such code paints an incomplete picture as the permissive semantics of dynamic languages can lead to puzzling runtime behavior.
Another option is to use fuzzers to generate new inputs.
However, fuzzing in the context of dynamic languages is tricky, as the shape of user-defined data is difficult to automatically determine ahead-of-time.
...
Unlike traditional use of fuzzing,\Ie to discover bugs and preformance patologies, our aim to find new successfull function invocation that can extend the scope of a program analysis.
...
Concertely, we stress-test one of the proposed type system designs for R, where function types were inferred from recorded call signatures from functions tests and examples.
Fuzzing \UFNumFunctions of those R functions, \tool generated \UFSignatrAdditionalSignatures (\UFSignatrAdditionalSignaturesRatio times) more unique call signatures in comaprison to the original study.
