...
Most of them \TODO{@pierre - do we know how many?} contain attributes which is what makes them interesting as the attributes bring additional semantic meaning.
...
While the aim of this work is not bug finding, we have investigated one such a crash that appeared in \code{stringi}\footnote{A sting processing library, one of the most downloaded package in R.}.
...
\FK{Say that in the end we want to use both - tracing and fuzzing.}
\FK{Do we define what is a call signature? Probably it is obvious, but just in case?}
\FK{We should define what is a successful call}
