...
We present the insights of its design and implementation, and assess \tool's ability to uncover new and interesting function calls by fuzzing \UFNumFunctions R functions and inferring their type based on the successful calls, revealing \UFSignatrAdditionalSignatures (\UFSignatrAdditionalSignaturesRatio times) more unique call signatures than existing code alone.
...
Fuzzing \UFNumFunctions of those R functions, \tool generated \UFSignatrAdditionalSignatures (\UFSignatrAdditionalSignaturesRatio times) more unique call signatures compared to the original study.
...
THe database maintains tables for the runtime metadata (such as the number of calls a value has been seen), the static metadata, the origins, call ids and class names. Origin strings and class names are interned. It also builds search indexes using fast compressed bitsets\cite{chambi2016better}.
