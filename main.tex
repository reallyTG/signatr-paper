...
On average, there is 7x slowdown when running the R code with tracing enabled. The main cost comes from value serialization (\Cf Section~\ref{sec:sxpdb}).
The tracer is written in 600 lines of C++ code.

\FK{Add information about the overhead}
...
The database provides a convenient query API based on this information, which represents search parameters for the database and is comprised of the {\tt typeof}-type, \AT{list all}, and length of the value.
