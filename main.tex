...
Retrofitting type systems onto dynamic languages can help programmers write more reliable code, but designing a type system that accurately captures the semantics of the language while being usable and useful to programmers can be tricky, as the balance between expressiveness and complexity is elusive.
A good starting point is to design a system based on existing code, perhaps by leveraging insights from a corpus analysis, or by inferring types from program executions in multiple candidate type system designs.
...
In this work, we propose an approach that uses fuzzing to obtain new, unexpected program traces to stress-test candidate retrofitted type system designs.
...
It contains \DBValuesRnd unique values recorded from \TODO{XXX - @pierre} calls to \DBNumFunctionsRnd functions from \DBNumPackages packages.
\FK{info about value distribution?}
